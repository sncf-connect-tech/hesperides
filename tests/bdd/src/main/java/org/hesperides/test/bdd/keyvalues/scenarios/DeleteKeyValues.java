...
public class DeleteKeyValues extends HesperidesScenario implements En {
    ...
    public DeleteKeyValues() {

        When("^I delete this key value$", () -> {
            keyValueClient.deleteKeyValue(keyValueBuilder.getId());
        });

        Then("^the key value is successfully deleted$", () -> {
            assertNoContent();
            keyValueClient.getKeyValue(keyValueBuilder.getId(), "should-fail");
            assertNotFound();
        });
    }
}
