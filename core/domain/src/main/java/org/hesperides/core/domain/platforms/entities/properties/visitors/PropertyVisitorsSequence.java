...
public class PropertyVisitorsSequence {
    ...
    public Stream<ModuleDetailedPropertyView> toModuleDetailedProperties(String propertiesPath, List<DetailedPropertyView> globalProperties, Set<String> unusedProperties) {
        return properties.stream()
                // On ne gère pour l'instant que les propriétés simples
                .filter(SimplePropertyVisitor.class::isInstance)
                .map(SimplePropertyVisitor.class::cast)
                .map(simplePropertyVisitor -> simplePropertyVisitor.toModuleDetailedProperty(propertiesPath, globalProperties, unusedProperties));
    }
