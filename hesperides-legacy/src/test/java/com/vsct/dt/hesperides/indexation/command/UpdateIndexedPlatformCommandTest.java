...
public class UpdateIndexedPlatformCommandTest {
    ...
    @Test
    public void shouldSendBodyJSONPlatformToUpdateUrlWithPlatformID() throws JsonProcessingException {
        PlatformIndexation platform = new PlatformIndexation("name", "appName", "appVersion", Lists.newArrayList());

        new UpdateIndexedPlatformCommand(platform).index(elasticSearchClient);

        verify(executer).post("/platforms/" + platform.getId() + "/_update?fields=_source", MAPPER.writeValueAsString(DocWrapper.of(platform)));
    }
