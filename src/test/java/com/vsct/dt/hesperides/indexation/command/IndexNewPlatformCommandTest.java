...
public class IndexNewPlatformCommandTest {
    ...
    @Test
    public void shouldSendJSONBodySringToPlatformUrlWithPlatformId() throws JsonProcessingException {
        PlatformIndexation platform = new PlatformIndexation("name", "appName", "appVersion", Lists.newArrayList());

        new IndexNewPlatformCommand(platform).index(elasticSearchClient);

        verify(executer).post("/platforms/" + platform.getId(), MAPPER.writeValueAsString(platform));
    }
}
