 ...
 */
public final class UpdateIndexedTemplateCommand implements ElasticSearchIndexationCommand<TemplateIndexation> {

    private static final Logger LOGGER = LoggerFactory.getLogger(UpdateIndexedTemplateCommand.class);

    private final TemplateIndexation template;

    public UpdateIndexedTemplateCommand(final TemplateIndexation template) {
        this.template = template;
    }

    @Override
    public Void index(final ElasticSearchClient elasticSearchClient) {
        String url = String.format("/templates/%1$s/_update?fields=_source", template.getId());
        ...
        elasticSearchClient.withResponseReader(ElasticSearchMappers.ES_ENTITY_TEMPLATE_READER).post(url, templateAsJson);

        LOGGER.info("Successfully updated template {}", template);
        return null;
    }
}
